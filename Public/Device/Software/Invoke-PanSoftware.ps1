      ...
      foreach($DeviceCur in $PSBoundParameters.Device) {
         ...
         if($PSCmdlet.ParameterSetName -eq 'Info') {
            ...
            if($R.Status -eq 'success') {
               ...
               Write-Error ('Error getting currently known software updates. Status: {0} Code: {1} Message: {2}' -f $R.Status,$R.Code,$R.Message)
            }
         ...
         if($PSCmdlet.ParameterSetName -eq 'Check') {
            ...
            if($R.Status -eq 'success') {
               ...
               Write-Error ('Software check not successful. Status: {0} Code: {1} Message: {2}' -f $R.Status,$R.Code,$R.Message)
            }
         } # End ParameterSetName
         ...
         if($PSCmdlet.ParameterSetName -eq 'Download') {
            ...
            if($R.Status -eq 'success') {
               ...
               Write-Error ('Software download not successful. Status: {0} Code: {1} Message: {2}' -f $R.Status,$R.Code,$R.Message)
            }
         } # End ParameterSetName
         ...
         elseif($PSCmdlet.ParameterSetName -eq 'Install') {
            ...
            $R = Invoke-PanXApi -Device $DeviceCur -Op -Cmd $Cmd
               if($R.Status -eq 'success') {
                  ...
                  Write-Error ('Software install not successful. Status: {0} Code: {1} Message: {2}' -f $R.Status,$R.Code,$R.Message)
               }
         } # End ParameterSetName
         ...
         elseif($PSCmdlet.ParameterSetName -eq 'Delete') {
            ...
            $R = Invoke-PanXApi -Device $DeviceCur -Op -Cmd $Cmd
               if($R.Status -eq 'success') {
                  ...
                  Write-Error ('Software delete failed. Status: {0} Code: {1} Message: {2}' -f $R.Status,$R.Code,$R.Message)
               }
         } # End ParameterSetName
      } # foreach Device
   } # Process block
