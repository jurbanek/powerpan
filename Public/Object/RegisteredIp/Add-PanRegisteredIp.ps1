...
"HerTag" and "HisTag" are both applied to 1.1.1.1 registered-ip.
   ...
   [CmdletBinding()]
   param(
      [parameter(Mandatory=$true,ValueFromPipeline=$true,HelpMessage='PanDevice against which IP registration and tagging will take place')]
      [PanDevice[]] $Device,
      [parameter(Mandatory=$true,Position=0,ParameterSetName='RegisterWithStrings',HelpMessage='IP address to register')]
      [String[]] $Ip,
      [parameter(Mandatory=$true,Position=1,ParameterSetName='RegisterWithStrings',HelpMessage='Tag(s) to apply to IP address')]
      [String[]] $Tag,
      [parameter(Mandatory=$true,Position=0,ParameterSetName='RegisterWithPanRegisteredIp',HelpMessage='PanRegisteredIp object to apply')]
...
'@
         ...
         if($PSCmdlet.ParameterSetName -eq 'RegisterWithStrings') {
            foreach($IpCur in $Ip) {
               Write-Verbose ($MyInvocation.MyCommand.Name + ":    registered-ip: $IpCur")
         ...
         elseif($PSCmdlet.ParameterSetName -eq 'RegisterWithPanRegisteredIp') {
            foreach($RegisteredIpCur in $RegisteredIp) {
               ...
               foreach($RegisteredIpCurTagCur in $RegisteredIpCur.Tag) {
                  Write-Verbose ($MyInvocation.MyCommand.Name + ":       tag: $RegisteredIpCurTagCur")
...
'@
         Write-Verbose ($MyInvocation.MyCommand.Name + ': Device: ' + $Device.Name)
