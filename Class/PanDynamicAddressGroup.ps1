class PanDynamicAddressGroup : PanObject {
   ...
   # [PanDevice] $Device
   # Override the XPath as hidden since it is irrelevant
   hidden [String] $XPath
   ...
   static PanDynamicAddressGroup() {
      ...
      'PanDynamicAddressGroup' | Update-TypeData -MemberName Name -MemberType ScriptProperty -Value {
      # Getter
      ...
      'PanDynamicAddressGroup' | Update-TypeData -MemberName Member -MemberType ScriptProperty -Value {
         # Getter
            # Different structures depending on NGFW and Panorama
            if($this.Device.Type -eq [PanDeviceType]::Ngfw) {
               $MemberAgg = @()
      ...
      'PanDynamicAddressGroup' | Update-TypeData -MemberName Filter -MemberType ScriptProperty -Value {
         # Getter
            # Different structures depending on NGFW and Panorama
            if($this.Device.Type -eq [PanDeviceType]::Ngfw) {
      ...
      'PanDynamicAddressGroup' | Update-TypeData -MemberName Location -MemberType ScriptProperty -Value {
         # Getter
            # Different structures depending on NGFW and Panorama
            if($this.Device.Type -eq [PanDeviceType]::Ngfw) {
         ...
         'PanDynamicAddressGroup' | Update-TypeData -MemberName Tag -MemberType ScriptProperty -Value {} -SecondValue {}
